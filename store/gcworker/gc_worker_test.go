...
func TestIssue33069(t *testing.T) {
	...
	_, err := se.Execute(context.Background(), `create database db1`)
	require.NoError(t, err)
	_, err = se.Execute(context.Background(), `create table db1.t1 (a int)`)
	require.NoError(t, err)
	_, err = se.Execute(context.Background(), `create table db1.t2 (a int)`)
	require.NoError(t, err)
	_, err = se.Execute(context.Background(), `create table db1.t3 (a int)`)
	require.NoError(t, err)
	_, err = se.Execute(context.Background(), `drop database db1`)
	require.NoError(t, err)
